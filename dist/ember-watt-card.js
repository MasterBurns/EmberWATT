	...
	_updatePaths() {
		...
		if (this._config.battery_entities && this._config.battery_entities.length > 0) {
			...
			this._config.battery_entities.forEach((e, t) => {
				...
				if (!f) return;
				let p = this._getEdges(f, t), m = this._getCenter(f, t), h = p.top + 16, g = !1;
				d[r].forEach((r) => {
					...
					if (l) {
						...
						if (o.invert_power && (p = -p), Math.abs(p) > 0 || u) {
							g = !0, n = !0;
							let t = f.top, l = a.bottom;
							c.push({
								id: `battery-${r}-path`,
								d: `M ${d.x} ${t} L ${d.x} ${h} L ${m.x} ${h} L ${m.x} ${s} L ${i.x} ${s} L ${i.x} ${l}`,
								power: Math.abs(p),
								color: o.color || e,
								reverse: !1
				...
				if (l) {
					...
					if (o.invert_power && (p = -p), Math.abs(p) > 0 || u) {
						...
						c.push({
							id: `battery-${r}-path`,
							d: `M ${d.x} ${t} L ${d.x} ${s} L ${i.x} ${s} L ${i.x} ${l}`,
							power: Math.abs(p),
							color: o.color || e,
							reverse: !1
						}), c[c.length - 1].reverse = p > 0;
